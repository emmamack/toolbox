...
def levenshtein_distance(s1,s2,cache):
    """Finds levenschtein distance between two given strings
    ...
    #updating cache with any new combination
    if (s1,s2[:-1]) not in cache:
        cache[(s1,s2[:-1])] = levenshtein_distance(s1,s2[:-1],cache)
    if (s1[:-1],s2) not in cache:
        cache[(s1[:-1],s2)] = levenshtein_distance(s1[:-1],s2,cache)
    if (s1[-1],s2[:-1]) not in cache:
        cache[(s1[:-1],s2[:-1])] = levenshtein_distance(s1[:-1],s2[:-1],cache)

    #deals with weird ai =! bj thing
    aibj = 0 if s1[-1] == s2[-1] else 1

    #calculates levenshtein distance, all cases now in cache
    return min(cache[(s1,s2[:-1])]+1,
                cache[(s1[:-1],s2)]+1,
                cache[(s1[:-1],s2[:-1])]+aibj)
...
def evaluate_text(message, goal_text, verbose=VERBOSE):
    ...
    """
    cache = {('', ''):0}
    distance = levenshtein_distance(message.get_text(), goal_text, cache)
    if verbose:
